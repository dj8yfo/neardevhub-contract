...
deploy_devhub: create_devhub_test_account
    pueue add -g neardevhub-contract cargo near deploy build-reproducible-wasm {{ devhub_test_contract }} with-init-call new \
...
deploy_community_factory: create_community_factory_subaccount
    cd community-factory && pueue add -g neardevhub-contract cargo near deploy build-reproducible-wasm {{ community_factory_contract }} \
...
_create_acc_and_deploy folder contract_account:
    ...
    cd {{ folder }} && pueue add -g neardevhub-contract cargo near deploy build-reproducible-wasm {{ contract_account }} \
        without-init-call network-config testnet \
        sign-with-keychain send
